-- Random weekly events: variasi kejadian kecil tiap minggu (moral, atribut,
-- cedera ringan, reputasi, keuangan) supaya dunia game terasa hidup.
-- Pesan disimpan di tabel fighter_news, ditampilkan di laporan minggu roster.

CREATE TABLE IF NOT EXISTS fighter_news (
...
ALTER TABLE fighter_news ENABLE ROW LEVEL SECURITY;

CREATE POLICY "fighter_news_select" ON fighter_news
  FOR SELECT USING (gym_id IN (SELECT id FROM gyms WHERE user_id = auth.uid()));

-- ─── Generate 0-2 kejadian acak per minggu ────────────────────────────────────
...
BEGIN
  ...
  FOR i IN 1..v_num_events LOOP
    ...
    v_idx := 1 + floor(random() * 22)::int;
    IF r.id IS NULL AND v_idx <= 18 THEN
      v_idx := 19 + floor(random() * 4)::int;
    END IF;
...
-- ─── advance_week(): tambah panggilan random fighter events ──────────────────
