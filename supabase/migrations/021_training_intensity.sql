-- Sistem intensitas latihan per fighter: low / medium / high
-- Low: growth -30%, tidak ada injury risk, fatigue tidak naik
-- Medium: growth normal, +5% injury risk, fatigue +5/minggu
-- High: growth +20%, +15% injury risk, fatigue +15/minggu

ALTER TABLE fighters
  ADD COLUMN IF NOT EXISTS training_intensity text NOT NULL DEFAULT 'medium'
  CHECK (training_intensity IN ('low', 'medium', 'high'));

-- Perbarui advance_week() untuk memperhitungkan intensitas
...
BEGIN
  ...
  -- Training load recovery (base), boosted by recovery room
  UPDATE fighters SET
    training_load = GREATEST(10, training_load - 5 - (v_recovery_level * 2))
  WHERE gym_id = p_gym_id
    AND status = 'training';

  -- Tambah fatigue berdasarkan intensitas (medium +5, high +15, low +0)
  UPDATE fighters SET
  ...
  -- Perkembangan atribut berdasarkan weekly_schedule + intensitas
  FOR r IN
  ...
  LOOP
    -- Multiplier growth berdasarkan intensitas
    ...
    -- Injury roll berdasarkan intensitas (hanya medium & high)
    ...
    -- Cek injury sebelum proses sesi
    IF v_injury_roll > 0 AND RANDOM() < v_injury_roll THEN
      ...
      WHERE id = r.id;
      CONTINUE; -- fighter cedera, lewati sesi latihan minggu ini
    END IF;
