-- Fight Camp: otomatis aktif saat next_fight_week 1-4 minggu ke depan
-- Efek: growth +10%, injury risk /2, morale +3/minggu, fatigue -3 extra/minggu
-- Tidak ada kolom baru — dideteksi dari next_fight_week vs season_week
...
BEGIN
  ...
  -- Perkembangan atribut + fight camp detection
  FOR r IN
  ...
  LOOP
    -- Deteksi fight camp: next_fight_week 1-4 minggu ke depan
    v_in_fight_camp := (
      r.next_fight_week IS NOT NULL AND
      (r.next_fight_week - v_season_week) BETWEEN 1 AND 4
    );

    -- Multiplier growth dari intensitas
    ...
    -- Injury roll dasar dari intensitas
    ...
    -- Fight camp: boost growth, kurangi injury risk, morale & recovery bonus
    ...
    -- Cek cedera sebelum proses sesi
    ...
    FOREACH v_day IN ARRAY v_days LOOP
      ...
      -- Baca nilai terkini dari DB (agar update berantai dalam minggu yang sama akurat)
  ...
  LOOP
    IF RANDOM() < 0.12 THEN
      ...
      BEGIN
        IF v_val > 30 THEN
          UPDATE fighters
            SET attrs = jsonb_set(attrs, ARRAY[v_key], to_jsonb(v_val - 1))
          WHERE id = r.id;
        END IF;
      END;
    END IF;
