-- Sistem Morale/Trust: indikator hubungan fighter dengan gym yang mempengaruhi
-- retensi kontrak.
--   - Morale naik/turun di app/game/fight/page.tsx berdasarkan hasil pertandingan
--     (menang/kalah, win bonus, title shot, cedera).
--   - Setiap minggu, overtraining (training_load >= 90) menurunkan morale,
--     sementara training_load rendah (<= 30) menaikkan morale.
--   - Saat kontrak habis (contract_fights_left <= 0), fighter dengan morale
--     rendah lebih besar peluangnya meninggalkan gym (alih-alih bertahan).

alter table fighters add column if not exists morale integer not null default 70;
...
begin
  ...
  -- Morale drift: overtraining menurunkan morale, istirahat cukup menaikkan
  update fighters set
    morale = greatest(0, morale - 2)
  where gym_id = p_gym_id and status = 'training' and training_load >= 90;

  update fighters set
    morale = least(100, morale + 1)
  where gym_id = p_gym_id and status = 'training' and training_load <= 30;
  ...
  -- Kontrak habis & tidak diperpanjang: peluang fighter pergi dari gym,
  -- dipengaruhi morale (morale rendah = lebih besar peluang pergi)
  with retired as (
    update fighters set status = 'retired'
    where gym_id = p_gym_id
      and status in ('training', 'active', 'prospect')
      and contract_fights_left <= 0
      and random() < (0.5 - (morale / 100.0) * 0.4)
