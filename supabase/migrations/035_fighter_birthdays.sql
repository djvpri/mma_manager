-- Tanggal lahir ringan per fighter: birth_week (1-52) dipakai untuk flavor
-- "Ulang Tahun" mingguan (morale boost kecil) di advance_week().

ALTER TABLE fighters ADD COLUMN IF NOT EXISTS birth_week integer;

-- Backfill fighter yang sudah ada dengan birth_week acak
UPDATE fighters SET birth_week = 1 + floor(random() * 52)::int WHERE birth_week IS NULL;

-- Default acak untuk fighter baru (pool prospect, recruit, CPU roster, dst — tanpa perlu ubah INSERT lain)
ALTER TABLE fighters ALTER COLUMN birth_week SET DEFAULT (1 + floor(random() * 52))::int;
ALTER TABLE fighters ALTER COLUMN birth_week SET NOT NULL;

-- ─── advance_week(): tambah birthday morale boost untuk fighter pemain ────────
