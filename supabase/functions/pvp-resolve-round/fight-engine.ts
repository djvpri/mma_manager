...
export interface FightConfig {
  ...
  /** PvP: game plan & corner advice pilihan LAWAN sendiri (bukan AI). Jika
   *  diisi, modifier lawan dihitung dari pilihannya sendiri secara simetris
   *  (bukan dari adaptasi AI prevOppPct). */
  oppGamePlan?: GamePlan
  oppCornerAdvice?: CornerAdvice
}
...
export function simulateRound(config: FightConfig): RoundResult {
  const { myFighter, opponent, gamePlan, cornerAdvice, roundNum, myStamina, oppStamina, myMental, oppMental, myHP, oppHP, prevOppPct, exchangeTicks, oppGamePlan, oppCornerAdvice } = config
  ...
  if (oppGamePlan && oppCornerAdvice) {
    // PvP: lawan punya pilihan sendiri, terapkan modifiernya secara simetris
    // (lawan men-buff dirinya & memberi efek balik ke saya, sama seperti saya ke dia)
    const [omm, oom] = GAME_PLAN_MODS[oppGamePlan]
    const [ocm, oco] = CORNER_MODS[oppCornerAdvice]
    opScore *= omm * ocm
    myScore *= oom * oco
  } else if (prevOppPct !== undefined) {
  ...
  // Survive: kalau pihak ybs kalah ronde ini, drastis kurangi risiko di-finish lawan.
  // All-out: risiko finish naik di kedua arah — lebih agresif tapi juga lebih terbuka.
  if (cornerAdvice === 'survive' && winner === 'opp') {
    finishChance *= 0.4
  } else if (cornerAdvice === 'allout') {
    finishChance *= winner === 'my' ? 1.5 : 1.4
  }
  if (oppCornerAdvice === 'survive' && winner === 'my') {
    finishChance *= 0.4
  } else if (oppCornerAdvice === 'allout') {
    finishChance *= winner === 'opp' ? 1.5 : 1.4
  }
  ...
  return {
    ...
    opp_stamina: clamp(oppStamina - (oppGamePlan && oppCornerAdvice
      ? myStaminaLoss(oppGamePlan, oppCornerAdvice, o.cardio, o.recovery)
      : oppStaminaLoss(o.cardio, o.recovery)), 0, 100),
    my_mental: clamp(myMental + mentalChange(winner === 'my', dmgToMe, myFinished, a.mental) + (cornerAdvice === 'survive' ? 4 : 0), 0, 100),
    opp_mental: clamp(oppMental + mentalChange(winner === 'opp', dmgToOpp, oppFinished, o.mental) + (oppCornerAdvice === 'survive' ? 4 : 0), 0, 100),
    myStats,
    oppStats,
    criteria: {
